...
def main():
    ...
    try:
        ...
        for row in csv_reader:
            user_id = row[0]
            flag = insert_db_points(rds_conn, user_id)
            if flag:
                insert_db_marketing(drds_conn, user_id)
                print('为 用户%s 赠送积分%s 成功' % (user_id,count))
            else:
                print('为 用户%s 赠送积分失败...' % (user_id,))
        # 结束时间
        end_time = time.time()
...
# 插入用户账户明细
def insert_db_marketing(conn, user_id):
